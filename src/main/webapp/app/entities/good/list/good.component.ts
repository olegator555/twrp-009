...
import { EntityArrayResponseType, GoodService } from '../service/good.service';
...
export class GoodComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.goods = this.refineData(dataFromBody);
  }

  protected refineData(data: IGood[]): IGood[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IGood[] | null): IGood[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.goodService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
